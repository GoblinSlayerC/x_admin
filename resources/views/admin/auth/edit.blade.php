...
<form class="layui-form">
    ...
    <dl class="mb30 groupname">
        ...
        <dd><input class="w500 text form-control" name="group_name" type="text" id="seller_group_name" value="{{$group->group_name}}" />
            <span></span>
            <p class="hint"></p>
        </dd>
    </dl>
            ...
            @foreach($menu_list as $key => $value)
                <div id="groupItem" class="mb15" style="display: flex;flex-direction: column;margin-bottom: 10px;">
                    ...
                    <label for="{{$key}}" style="display: flex;">
                        <input  id="{{$key}}" name="mids[]" value="{{$value->mid}}" @if($group->mid=='*' || in_array($value->mid,$group->mid))  checked    @endif bbctype="btn_select_module" type="checkbox" />
                        <b>{{$value->title}}</b>
                    </label>
                    <div class="submenulist" style="display: flex;">
                        ...
                        @foreach($value->child as $submenu_value)
                            <label for="{{$submenu_value->name}}" style="display: flex;">
                                <input id="{{$submenu_value->mid}}" name="limits[]" value="{{$submenu_value->mid}}" @if($group->mid=='*' || in_array($submenu_value->mid,$group->url))  checked    @endif type="checkbox" />
                                {{$submenu_value->title}}
                            </label>
    ...
    <div class="layui-form-item">
        ...
        <center><button class="layui-btn" lay-filter="edit" lay-submit="">修改</button></center>
    </div>
</form>
<script>
    $(function(){
